...
async def usuario_actual(
    ...
    usuario = await crud_usuarios.obtener(sesion, int(identificador))
...
def permitir_roles(*roles: str):
    ...
    async def comprobar(usuario: UsuarioActual) -> Usuario:
...
async def obtener_usuario_ruta(
    sesion: SesionDep, usuario_id: Annotated[int, Path(ge=1)]
) -> Usuario:
    return await crud_usuarios.obtener_o_fallar(sesion, usuario_id)


async def obtener_producto_ruta(
    sesion: SesionDep, producto_id: Annotated[int, Path(ge=1)]
) -> Producto:
    return await crud_productos.obtener_o_fallar(sesion, producto_id)


async def obtener_servicio_ruta(
    sesion: SesionDep, servicio_id: Annotated[int, Path(ge=1)]
) -> Servicio:
    return await crud_servicios.obtener_o_fallar(sesion, servicio_id)


async def obtener_cita_ruta(
