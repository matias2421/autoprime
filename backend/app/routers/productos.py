...
async def listar(
    ...
    productos = await crud_productos.listar(sesion, familia=familia, estado=estado)
...
async def crear(datos: ProductoCrear, sesion: SesionDep, _: Personal) -> SobreProducto:
    producto = await crud_productos.crear(sesion, datos.model_dump())
...
async def obtener(
    ...
    if identificador.isdigit():
        producto = await crud_productos.obtener_o_fallar(sesion, int(identificador))
    else:
        producto = await crud_productos.obtener_por_slug(sesion, identificador)
    return SobreProducto(producto=ProductoSalida.desde_modelo(producto))
...
async def actualizar(
    ...
    actualizado = await crud_productos.actualizar(sesion, producto, cambios)
...
async def eliminar(
    ...
    await crud_productos.eliminar(sesion, producto)
