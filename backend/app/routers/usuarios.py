...
async def registro_publico(datos: UsuarioRegistro, sesion: SesionDep) -> Sesion:
...
async def listar(
    ...
    usuarios = await crud_usuarios.listar(sesion, rol=rol, estado=estado, buscar=buscar)
...
async def crear(datos: UsuarioCrear, sesion: SesionDep, _: SoloAdmin) -> SobreUsuario:
    """Alta desde el panel: aquí sí se puede elegir el rol."""
    cuerpo = datos.model_dump()
    rol = cuerpo.pop("rol")
    usuario = await crud_usuarios.crear(sesion, cuerpo, rol_nombre=rol)
...
async def obtener(usuario: UsuarioRuta, _: SoloAdmin) -> SobreUsuario:
    return SobreUsuario(usuario=UsuarioSalida.desde_modelo(usuario))
...
async def actualizar(
    ...
    actualizado = await crud_usuarios.actualizar(sesion, usuario, cambios)
...
async def cambiar_estado(
    ...
    actualizado = await crud_usuarios.cambiar_estado(sesion, usuario, datos.estado)
    return SobreUsuario(usuario=UsuarioSalida.desde_modelo(actualizado))
...
async def eliminar(
    ...
    await crud_usuarios.eliminar(sesion, usuario)
